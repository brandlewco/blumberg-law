    ...
    return links.map((link, index) => {
        ...
        return (
            ...
            <li key={index}>
                {linkChild.length > 0 && (
                    <>
                        <div onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}><Action {...link} className={classNames(inMobileMenu && link.type === 'Button' ? 'w-full' : '')} data-sb-field-path={`.${index}`} /></div>
                        {open &&
                            <ul>
