...
function listOfLinks(links, inMobileMenu = false) {
    return links.map((link, index) => {
        ...
        return (
            <>
                {linkChild.length > 0 && (
                    <li key={index} data-key={index} data-url={isMobile ? "true" : "false"} className={classNames('dropdown', 'relative', 'block', 'lg:py-12')}
                        onMouseEnter={() => setIsMenuOpen(true)}
                        onMouseLeave={() => setIsMenuOpen(false)}
                        onMouseDown={() => setIsMenuOpen(toggle = !toggle)}
                    >
                        <Action {...link} url={isMobile ? "#" : link.url} style="link" showIcon="true" icon="menuDown" className="w-full flex flex-row justify-between border-b-2 lg:border-0 border-solid border-secondary " data-sb-field-path={`.${index}`} />
